...
class FoodController extends Controller
{
    ...
    public function pending(Request $request)
    {
        ...
        $notifications = FoodApprovalNotification::with(['food.restaurant', 'food.category.parent', 'vendor'])
            ->where('type', 'pending')
            ->when(is_numeric($restaurant_id), function ($query) use ($restaurant_id) {
                return $query->whereHas('food', function ($q) use ($restaurant_id) {
                    $q->where('restaurant_id', $restaurant_id);
                });
            })
            ->when(is_numeric($category_id), function ($query) use ($category_id) {
                return $query->whereHas('food.category', function ($q) use ($category_id) {
                ...
                $q->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->orWhere('message', 'like', "%{$value}%")
                            ->orWhereHas('food', function ($query) use ($value) {
                                $query->where('name', 'like', "%{$value}%");
                            });
                    }
                });
        ...
        return view('admin-views.product.pending_list', compact('notifications', 'restaurant', 'category', 'type'));
    }
    ...
    public function approveFood(Request $request, $id)
{
    ...
    $food->status = 1; // approved
    $food->save();

    FoodApprovalNotification::where('food_id', $id)
        ->where('type', 'pending')
        ->update([
            'type' => 'approved',
            'is_read' => false,
    ...
    Toastr::success(translate('Food approved successfully'));
    return redirect()->route('admin.food.pending');
}


    ////Denied food
    public function denyFood(Request $request, $id)
{
    $request->validate([
    ...
    Toastr::error(translate('Food denied successfully'));
    return redirect()->route('admin.food.pending');
}
    ...
    {
        $count = FoodApprovalNotification::pending()->unread()->count();
